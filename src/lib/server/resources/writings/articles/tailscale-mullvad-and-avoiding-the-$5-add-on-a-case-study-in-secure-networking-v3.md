...
Here, we can see my local IP addresses (`10.74.58.18` and `fc00:bbbb:bbbb:bb01::b:3a11`), DNS resolver (`100.64.0.7`), and the server to use for tunneling (`143.244.47.65`). The server shares the same address space as my mapped address (`143.244.47.0/24`, i.e., `143.244.47.xxx`), so the destination sees it, rather than the endpoint. As a point of concern, note that Mullvad and the trail of servers to the destination can read the traffic. [While I trust Mullvad](https://mullvad.net/en/vpn/trustworthy-vpn), it’s best to encrypt the traffic end-to-end with protocols like HTTPS.

This works for accessing the Internet after the tunnel has been established, but not for the home network because Internet VPNs don’t route between their clients and local addresses. For example, if my iPhone ran a server on port 50000, my MacBook Pro couldn’t access it: 
...
The VPN server can’t route to my iPhone without passing through the router. If it tried to, the tunnel data would have to be decrypted beforehand, allowing anyone on my home network to read my traffic. This defeats the purpose of using a VPN, [so services like Mullvad allow you to access the local network by routing traffic outside the tunnel](https://mullvad.net/en/help/using-mullvad-vpn-app#lan-share), making it visible:
...
This works, but exposes the server to the Internet, which may be unwanted when it shouldn’t be public (e.g., you have to keep it up-to-date for security patches). What we really want is a way to access the server in a private network only select devices know how to access, while also encrypting the traffic without relying on [self-signed certificates](https://en.wikipedia.org/wiki/Self-signed_certificate). In other words, we want our connections to look more like so:
...
Notice that my 2019 MacBook Pro and iPhone transfer data to the same M6 MacBook Pro, but don’t transfer between each other. This allows connections to be end-to-end encrypted, preventing other devices on the network from reading the traffic.
...
The next step is to set up the reverse proxy server. You could use [nginx](https://nginx.org), but Caddy is simpler and enables automatic HTTPS. While the tunnel is already secure, browsers base their security on their own connections (HTTP, HTTPS, localhost, etc.), so some features require HTTPS (e.g., autofilling passwords). [This is how I’ve defined my Caddyfile:](https://github.com/kyleerhabor/nix-config/blob/6dddf51638af62695081dafaf16e041256ea7bbe/hosts/kyles-macbook-pro/servers/caddy/resources/Caddyfile)
...
[You can explore my Nix configuration if you’re interested in how I run Caddy and other servers on my MacBook Pro.](https://github.com/kyleerhabor/nix-config/tree/6dddf51638af62695081dafaf16e041256ea7bbe) The result is that I can access Navidrome at `navidrome.home.kyleerhabor.com` on my MacBook Pro and iPhone:
...
The IPv4 address `100.103.176.102` and IPv6 address `fd7a:115c:a1e0::a337:b067` match. Now, let me check while being disconnected from Tailscale:
...
* IPv4: 100.103.176.102
*   Trying 100.103.176.102:443...
*   Trying [fd7a:115c:a1e0::a337:b067]:443...
* connect to fd7a:115c:a1e0::a337:b067 port 443 from fc00:bbbb:bbbb:bb01::b:3a11 port 65175 failed: Network is unreachable
* connect to 100.103.176.102 port 443 from 10.74.58.18 port 65174 failed: Operation timed out
* Failed to connect to navidrome.home.kyleerhabor.com port 443 after 75056 ms: Couldn't connect to server
* Closing connection
curl: (28) Failed to connect to navidrome.home.kyleerhabor.com port 443 after 75056 ms: Couldn't connect to server
```

This says that Mullvad doesn’t know how to access my private network, but applies to all connections outside the Tailscale tunnel, confirming that routing works. In case you’re curious, [`fe80::aede:48ff:fe00:1122` is a link-local address](https://en.wikipedia.org/wiki/Link-local_address) and operates lower in the stack.

## Limitations

This setup makes running multiple VPNs simultaneously a viable option, but is not without its limitations.

### Kill switch

A killer feature in VPN apps is a kill switch to prevent traffic bound for the tunnel from leaking outside it. WireGuard’s app doesn’t have this, so you may notice DNS leaks from tests like [IP Leak](https://ipleak.net) (try switching tunnels while it’s active). A workaround is to set a [SOCKS5 proxy](https://mullvad.net/en/help/socks5-proxy) so disconnecting from Mullvad makes connecting to the proxy fail, but this requires cooperation from the software you’re using (e.g., [curl](https://curl.se) requires you to specify the proxy). I think a long-term solution would be to add a fall-through VPN that drops the connection, but I’m not aware of any app for this.
...
[Tailscale exit nodes](https://tailscale.com/docs/features/exit-nodes) act like Internet VPNs, routing traffic from the default route to a server which forwards traffic (here, another device on the private network). Consequently, they conflict with other VPNs interested in the same route (here, Mullvad). I don’t need this feature, but if I did, I’d opt for the $5 add-on.

## Conclusion

At the beginning, I didn’t use a VPN. This was fine for most connections, but meant that my Internet traffic was visible to whoever managed the network, and my local traffic was visible to anyone on the same network. After I got an Internet VPN, my connections from my device to the VPN server were secure, but connections between my devices remained insecure. Later, I got a mesh VPN that secured those connections, but didn’t integrate with my setup, forcing me to toggle. The setup I’ve explored here, while imperfect, is a massive step up from remembering to connect and disconnect VPNs. I can access Navidrome at `https://navidrome.home.kyleerhabor.com` and other servers from all my devices without being concerned about security, device hostnames, IP addresses, port numbers, etc.

This article took about a week to write, and yet I feel like I garnered the same amount of knowledge as I did in my computer networks course over 15 weeks. I recognize that most professors are invested in research and the limits to what you can learn in a YouTube series, but think most students can benefit from getting their hands dirty. The result is that I upgraded my setup, all while saving myself from spending $5 on an add-on that isn’t strictly necessary.
