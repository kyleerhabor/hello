...
As for the dependencies themselves, Nix represents each one as a derivation—a build recipe that encodes its inputs, build steps, and outputs. In practice, I use `stdenv.mkDerivation`, which orchestrates the build lifecycle for C projects. By default, it looks for `./configure`, runs `make`, and finally `make install`. This allows me to provide the source, list the build tools, and let Nix manage the rest. For example, [Ogg is defined like so](https://github.com/kyleerhabor/sampled/blob/5e7415c392eeb1d09b6fc5825224963cc1dad2f7/nix/packages/libogg.nix):
