...
export async function load({ params }) {
  ...
  const article = data[server.KEY_ARTICLES]
    .find((article) => article[server.KEY_ARTICLE_ID] === params.id);
  ...
  const rendered = await render(
    modules[`/src/lib/server/resources/writings/articles/${article[server.KEY_ARTICLE_CONTENT]}`],
  );

  const result = {
    ...renderArticle(article),
    [client.KEY_DATA_ARTICLE_DESCRIPTION]: article[server.KEY_ARTICLE_DESCRIPTION],
