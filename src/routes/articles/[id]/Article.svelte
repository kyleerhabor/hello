    ...
    <article class="article-content">
      <div class="header">
        <div>
          ...
          <span class="meta">
            {const oldestVersion = $derived(data[server.KEY_DATA_ARTICLE_VERSIONS][0])}
            {const isOldest = $derived(
              data[server.KEY_DATA_ARTICLE_VERSION] === oldestVersion[server.KEY_DATA_ARTICLE_VERSION_NUMBER]
            )}
            Published
